...
/* Array donde se almacenan los eventos pasados y futuros despues del filtro   */  

let pastEvents = [];
let upEvents = [];

/* Funcion Que recibe una respuesta "data" de una API, y que hace un filtro en eventos pasados y futuros para almacernarlos en sus respectivos arrays */
function filterPastEvents(array) {
    for (let i = 0; i < array.length; i++) {
        if (array[i].date < currentDate) {
            ...
            upEvents.push(array[i])
        }
    }
}
    filterPastEvents(events)

function insertPastCards(array){
    let body = ``;
    let containPastUpdate = document.getElementById("past_events");
        for(let i=0; i < array.length; i++){
            body += `
          <div class="col col-card">
            <div class="card" style="width: 18rem;">
              <img id="img-card" src="${array[i].image}" class="card-img-top" alt="${array[i].category}">
              <div class="card-body">
        ...
        containPastUpdate.innerHTML = body;
    }
insertPastCards(pastEvents);



/* Funcion para insertar dinamicamente las cards correspondientes al carrousel responsive de Boostrap 5 */

function insertcarouselPastCards(array){
  let carouselPastUpdate = document.getElementById("carousel-inner");
  let carousel = ``;
  for(let x = 0; x < 1; x++){
...
for(let i=1; i < array.length; i++){
      ...
      carouselPastUpdate.innerHTML = carousel;
  }
  insertcarouselPastCards(pastEvents);
  ...
  function insertNull(array){
    ...
    let carouselPastUpdate = document.getElementById("carousel-inner");
        ...
        containPastUpdate.innerHTML = body;
        carouselPastUpdate.innerHTML = body
    }
  ...
  search.addEventListener("keyup", ()=> {
    let arrayFiltrado = filterText(pastEvents, search.value);
    let arrayFiltradoDos = filterCheck(arrayFiltrado);
    if(arrayFiltradoDos != 0){
      insertcarouselPastCards(arrayFiltradoDos);
    insertPastCards(arrayFiltradoDos);
    }else{
      insertNull(eventNull);
      insertNull(eventNull);
    }
  ...
  searchDos.addEventListener("keyup", ()=> {
    let arrayFiltrado = filterText(pastEvents, searchDos.value);
    let arrayFiltradoDos = filterCheck(arrayFiltrado);
    if(arrayFiltradoDos != 0){
      insertcarouselPastCards(arrayFiltradoDos);
    insertPastCards(arrayFiltradoDos);
...
/* let search = events.filter(element => element.date >= "2021-11-02" && element.date <= "2022-01-22")

/* search.forEach(element => console.log(element.name));  */
